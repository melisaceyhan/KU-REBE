...
namespace OpenCVForUnityExample
{
    ...
    public class MultiObjectTrackingBasedOnColorExample : MonoBehaviour
    {
        ...
        public UnityEvent TargetdetectedOrder;

        public UnityEvent DontTargetdetectedOrder;
        ...
        private void drawObject(List<ColorObject> theColorObjects, Mat frame, Mat temp, List<MatOfPoint> contours, Mat hierarchy)
        {
            for (int i = 0; i < theColorObjects.Count; i++)
            {
                ...
                if (theColorObjects[i].getType().ToString() == ColorTarget)
                {
                    if (TargetdetectedOrder != null)
                    {
                        TargetdetectedOrder.Invoke();
                    }
                }

                if (theColorObjects[i].getType().ToString() != ColorTarget)
                {
                    if (DontTargetdetectedOrder != null)
                    {
                        DontTargetdetectedOrder.Invoke();
                    }
                }
